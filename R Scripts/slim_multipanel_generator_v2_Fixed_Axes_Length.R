...
main <- function(input_paths, ext) {
  ...
  if (kind == "trajectory") {
    # First pass: read each file briefly to find global x_max (generations / 1000)
    # and global y_max (we use 1.1 as a fixed floor since intrinsic fitness can
    # rise above 1, e.g., the Darwinian demon case, but we want a clean limit).
    all_x_max <- 0
    all_y_max <- 1.1
    for (p in input_paths) {
      df <- read.csv(p, check.names = FALSE, stringsAsFactors = FALSE)
      require_columns(df, c("cycle", "Mean_fitness", "Intrinsic_fitness"))
      all_x_max <- max(all_x_max, max(df[["cycle"]] / 1000, na.rm = TRUE))
      all_y_max <- max(all_y_max,
                       max(df[["Mean_fitness"]], na.rm = TRUE) * 1.05,
                       max(df[["Intrinsic_fitness"]], na.rm = TRUE) * 1.05)
    }
    panels <- lapply(input_paths, build_trajectory_panel,
                     y_max = all_y_max, x_max = all_x_max)
  } else {
