# =====================================================================
# DFE Comparison Plot (ENZ + SBZ Shaded) — v14 JoMB Styling
# =====================================================================
# Shows the gamma distributions of fitness effects for H&C's baseline
# and three B1 runs isolating mean fitness effects. Two zones are shaded:
#   - Effectively neutral zone (ENZ, |s| < 0.0005)         - pale green
#   - Selection breakdown zone (0.0005 <= |s| <= 0.0025)   - pale orange
#
...
# ---------- Zone boundaries --------------------------------------------
ENZ_THRESHOLD <- 0.0005   # ENZ upper bound: |s| < 1/(2N) for N=1000
SBZ_THRESHOLD <- 0.0025   # SBZ outer bound: |s| <= 0.0025
...
p1 <- ggplot(curve_data, aes(x = s, y = density, color = run)) +
  ...
  # Shade both ENZ (pale green) and SBZ (pale orange). We construct a 2-row
  # data frame with both zone bounds and map the fill aesthetic to a label
  # string; scale_fill_manual assigns the actual colors and the legend
  # entries appear automatically.
  geom_rect(data = data.frame(
              xmin = c(-ENZ_THRESHOLD, -SBZ_THRESHOLD),
              xmax = c(0,              -ENZ_THRESHOLD),
              ymin = c(0,              0),
              ymax = c(Inf,            Inf),
              fill_label = c(
  ...
  scale_fill_manual(
    name = NULL,
    values = c(
      "Effectively neutral zone (|s| < 0.0005)" = "#C8E6B8",        # pale green
      "Selection breakdown zone (0.0005 ≤ |s| ≤ 0.0025)" = "#FFCC99"  # pale orange
    ),
    breaks = c(
      "Effectively neutral zone (|s| < 0.0005)",
      "Selection breakdown zone (0.0005 ≤ |s| ≤ 0.0025)"
  ...
  # Dashed boundary line at the OUTER edge of the SBZ (at s = -0.0025)
  geom_vline(xintercept = -SBZ_THRESHOLD,
             linetype = "dashed", color = "#E07000",
             linewidth = 0.4, alpha = 0.8) +
  ...
  annotate("text",
           ...
           family = JOMB_FONT) +
  annotate("text",
           x = -(ENZ_THRESHOLD + SBZ_THRESHOLD) / 2,   # center of orange zone
           y = 8,                                       # bottom of orange zone
           label = "SBZ",
           color = "#B05000",
           size  = 5,
           fontface = "bold",
...
ggsave("DFE_comparison_ENZ_SBZ.png",
       plot   = p1,
       width  = 10,
       height = 6,
       dpi    = 300,
...
print(p1)
cat("Saved: DFE_comparison_ENZ_SBZ.png\n")
